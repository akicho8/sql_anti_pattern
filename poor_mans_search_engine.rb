...
# >>    (11.3ms)  DROP DATABASE IF EXISTS `__test__`
# >>    (0.5ms)  CREATE DATABASE `__test__` DEFAULT CHARACTER SET `utf8`
# >>    (15.3ms)  CREATE TABLE `articles` (`id` int AUTO_INCREMENT PRIMARY KEY, `body` varchar(255),  INDEX `index_articles_on_body`  (`body`)) ENGINE=InnoDB
# >>    (9.3ms)  CREATE TABLE `ar_internal_metadata` (`key` varchar(255) PRIMARY KEY, `value` varchar(255), `created_at` datetime NOT NULL, `updated_at` datetime NOT NULL) ENGINE=InnoDB
...
# >>    (0.2ms)  BEGIN
# >>   SQL (0.3ms)  INSERT INTO `ar_internal_metadata` (`key`, `value`, `created_at`, `updated_at`) VALUES ('environment', 'default_env', '2017-02-09 01:43:12', '2017-02-09 01:43:12')
# >>    (0.7ms)  COMMIT
# >>    (0.2ms)  BEGIN
...
# >>    (0.3ms)  COMMIT
# >>   Article Load (0.2ms)  SELECT `articles`.* FROM `articles` WHERE (body like '%日本%')
# >>   Article Load (0.2ms)  SELECT `articles`.* FROM `articles` WHERE (body regexp '[[:<:]]日本[[:>:]]')
...
# >> +----+-------------+----------+------------+-------+---------------+------------------------+---------+------+------+----------+--------------------------+
# >> 1 row in set (0.00 sec)
# >>    (1.4ms)  show variables like 'innodb_ft_min_token_size'
# >>    (127.3ms)  ALTER TABLE articles ADD FULLTEXT(body)
# >>   Article Load (0.4ms)  SELECT `articles`.* FROM `articles` WHERE (MATCH(body) AGAINST ('日本' IN BOOLEAN MODE))
